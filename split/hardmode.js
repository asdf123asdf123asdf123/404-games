    ...
    for (let i = 0; i < points.length; i++) { //遍历粒子
		...
		if(point.aaa>=1){
			...
			points.push({
	            ...
	            sy: point.sy,
				aaa:point.aaa-1,
	            vx: Math.sin(deg*Math.PI*2)*3*point.aaa, //x轴及y轴的移动向量，取值范围为-0.5 ~ 0.5
	            vy: Math.cos(deg*Math.PI*2)*3*point.aaa,
	            life: live*100,
    ...
    if(cnt%50==0){
    	...
    	for(let i=1;i<=t;i++){
	        ...
	        points.push({
	            sx: rx,
	            sy: ry,aaa:2,
	            vx: Math.sin(deg*Math.PI*2)*10, //x轴及y轴的移动向量，取值范围为-0.5 ~ 0.5
	            vy: Math.cos(deg*Math.PI*2)*10,
	            life: live*100,
	            color:colors[parseInt(Math.random() * colors.length)],
	            size:10,rt:0
	        })
